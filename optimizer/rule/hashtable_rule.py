...
class HashTableRule(DataStructureRule):
    ...
    def match_dynamic(self, node: Plan, choice_nodes: list[str], check_choice_input=True) -> bool:
        ...
        return all(map(
            lambda c: isinstance(c, Op) and c.op == "=" and \
                      not c.operands[0].find_exprs(lambda e: isinstance(e, ChoiceExpr)) and \
                      not c.operands[1].find_exprs(lambda e: isinstance(e, C)) and \
                      c.operands[1].find_exprs(lambda e: isinstance(e, ChoiceExpr) and e.choice_id in choice_nodes),
            conds))
    ...
    def apply(self, node: Plan, scache: bool):
        ...
        queries = [c.operands[1] for c in conds]

        ds = node.input.hashtable_build(keys)
        ds = ds.scache() if scache else ds.dcache()
        ds = ds.hashtable_query(queries)
