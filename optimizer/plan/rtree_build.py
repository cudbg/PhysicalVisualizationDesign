...
class RTreeBuild(Plan):
    ...
    def clone(self):
        return RTreeBuild(self.input.clone(), [key.clone() for key in self.keys])

    def bind(self, binding):
        return RTreeBuild(self.input.bind(binding), [key.bind(binding) for key in self.keys])
    ...
    def to_cost(self, switchon):
        ...
        avg_latency = self.latency("RTreeBuild", avg_input_n_rows, avg_output_n_rows, column_info)
        upper_latency = self.latency("RTreeBuild", upper_input_n_rows, upper_output_n_rows, column_info)
        upper_latency = max(upper_latency, avg_latency)
        mem = self.memory("RTreeBuild", upper_output_n_rows, column_info)
    ...
    def to_str(self):
        return f"RTreeBuild[{self.cost(False)[0].upper_latency}]({','.join([str(key) for key in self.keys])})\n|\n" + str(self.input)

    def to_functional_str(self):
        return f"RTreeBuild({','.join([str(key) for key in self.keys])})" + self.input.functional_str()

    def to_hash(self):
        return hash(("RTreeBuild", hash(self.input), (tuple(str(key) for key in self.keys))))
