...
class SCache(Plan):
    ...
    def clone(self):
        return SCache(self.input.clone())

    def bind(self, binding):
        return SCache(self.input.bind(binding))
    ...
    def to_cost(self, switchon):
        ...
        cost = Cost(0, 0, mem)
        stat = Statistics(avg_input_n_rows, upper_input_n_rows, input_stat.n_cols)
        return cost, stat

    def to_str(self):
        return f"SCache[{self.get_memory()}]\n|\n" + str(self.input)
    ...
    def to_hash(self):
        return hash(("SCache", hash(self.input)))

    def sig(self):
        return "s_" + str(hash(f"{self.at_server()}" + self.functional_str()))
